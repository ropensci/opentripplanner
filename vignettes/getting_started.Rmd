...
You will need to have installed R, RStudio, and Java 8.  If possible the 64 Bit version of Java is preferable, especially if you want to use OTP over large areas. If you are unfamiliar with R, Java, and Open Trip Planner see the Dummies Guide vignette incuded with this package.
...
# path_otp <- "~/programs/otp.jar" # On Linux
# path_otp <- "C:/Users/Public/Downloads/otp.jar" # On Windows
...
download.file("https://github.com/ITSLeeds/opentripplanner/releases/download/0.1/isle-of-wight-demo.zip", 
              destfile = "isle-of-wight-demo.zip")
...
Create a router called 'default', and include the GTFS and OSM files for the road network and default transit schedules for Greater Manchester, along with `router-config.json`, as shown below:


```{r, engine='bash', eval=FALSE}
/otp                         # Your top folder for storing all OTP data
  /graphs                     
     /default                 # Sub folder with the name or the router
...
```{r, eval = FALSE}
route <- otp_plan(otpcon, fromPlace = c(50.64590, -1.17502), toPlace = c(50.72266, -1.15339))
```
