...
**Note** `otp_plan_batch` supports multi-core processing via the `ncores` input. Runnin on `ncores = 2` provides around 35% performance boost. However, you are unlikely to get a significant improvement (on typical computers) using more than 3 cores, as you will overload OTP.
