...
Now you have the OTP running on your computer you can let R connect to the OTP. `otp_connect()` creates an OTP connection object which will allow R to connect to the OTP. By default your local timezone is used, but if your are not in the UK you need to specify the UK timezone to get the correct results with the demo data.
