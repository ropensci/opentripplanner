...
By default, R will assign OTP 2GB of memory to build the graph. For larger areas, you may need more memory and can use the `memory` argument. For example, to allocated 10GB you would use.

```{r eval=FALSE}
log <- otp_build_graph(otp = path_otp, dir = path_data, memory = 10) 
```
