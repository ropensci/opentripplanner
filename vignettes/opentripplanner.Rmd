...
You can also use the `otp_connect()` function to make a connection to OTP on a remote server, for example [plannerstack](http://demo.planner.plannerstack.com/) run an OTP server for the Netherlands. See their [documentation](http://docs.plannerstack.org/en/latest/) for more details.

```{r, eval = FALSE}
otpcon <- otp_connect(hostname = "demo.planner.plannerstack.com",
                      router = "default", port = 80)
```
