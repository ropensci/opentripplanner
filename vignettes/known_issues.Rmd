...
Known bug that stops driving on any road with cycling infrastructure. https://groups.google.com/forum/#!topic/opentripplanner-users/BOv1J32k6Sc 

## Speeding up routing

If you are doing a large amount of routing conisder the following options.

1. Use multicore routing using `ncores` argumment of `otp_plan()`
2. Reduce the area, for example if your are only routing in one city you don't need a whole country graph.
3. Remove unneeded roads using [osmfilter](https://wiki.openstreetmap.org/wiki/Osmfilter) or [OSMtools](https://github.com/ITSLeeds/OSMtools) for example if you are only inrested in driving you can remove footpaths and cycleways. This will simplify your graph and povide a small performance boost.
4. Upgrading you computer, a new fast CPU with more cores will route faster.


## GTFS Data for the UK

Coming Soon see https://github.com/ITSLeeds/UK2GTFS
